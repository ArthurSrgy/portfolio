...
func (c *Camera) Update(characterPosX, characterPosY int) {
	...
	case FollowSmooth:
		c.updateFollowSmooth(characterPosX, characterPosY)
	case Bloque:
		c.updateBloque(characterPosX, characterPosY)
	}
}
...
// updateFollowSmooth est la mise-à-jour d'une caméra qui suit
// toujours le personnage de manière fluide. Elle prend en paramètres
// deux entiers qui indiquent les coordonnées du personnage et place
// la caméra au même endroit.
func (c *Camera) updateFollowSmooth(characterPosX, characterPosY int) {
	c.X = characterPosX
	c.Y = characterPosY
}

// updateCamera dans le cas où CameraMode est trop grand
func (c *Camera) updateBloque(characterPosX, characterPosY int) {
	c.X = characterPosX
	c.Y = characterPosY
}
